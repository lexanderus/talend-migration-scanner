...
async function generatePdf() {
  ...
  // Load jsPDF and html2canvas lazily
  await loadScript(JSPDF_CDN);
  await loadScript(H2C_CDN);

  const { jsPDF } = window.jspdf;
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
  const W = 210, H = 297, M = 15; // A4 dimensions, 15mm margins
  const date = new Date().toISOString().slice(0, 10);
  const projectName = scanResult.meta?.filename?.replace('.zip','') || 'project';
  const footer = `Generated by Talend Migration Scanner · lexanderus.github.io/talend-migration-scanner · ${date}`;
  ...
  doc.setTextColor(13, 27, 62);
  doc.text('Talend Migration Scanner', W / 2, 30, { align: 'center' });
  doc.setFontSize(14); doc.setTextColor(10, 147, 150);
  doc.text(projectName, W / 2, 40, { align: 'center' });
  doc.setFontSize(10); doc.setTextColor(100);
  doc.text(date, W / 2, 48, { align: 'center' });
  doc.setTextColor(0);

  // Big percentage
  doc.setFontSize(48); doc.setFont('helvetica', 'bold');
  doc.setTextColor(10, 147, 150);
  doc.text(`${scanResult.summary.auto_pct}%`, W / 2, 72, { align: 'center' });
  doc.setFontSize(12); doc.setTextColor(80);
  doc.text('jobs migrate automatically', W / 2, 80, { align: 'center' });
  doc.setTextColor(0); doc.setFont('helvetica', 'normal');

  // Summary table
  let y = 95;
  const sumRows = [
    ['AUTO',    String(scanResult.summary.auto)],
    ['PARTIAL', String(scanResult.summary.partial)],
    ['MANUAL',  String(scanResult.summary.manual)],
    ['SKIP',    String(scanResult.summary.skip)],
    ['Total',   String(scanResult.summary.total_jobs)],
  ];
  doc.setFontSize(10);
  for (const [label, val] of sumRows) {
    doc.text(label, M + 20, y); doc.text(val, M + 60, y);
    y += 7;
  }

  // Issues summary
  y += 5;
  y = header('Detected Issues', y);
  for (const [flag, count] of Object.entries(scanResult.issues_summary)) {
    if (count === 0) continue;
    doc.setFontSize(10);
    doc.text(`• ${flag}`, M + 5, y); doc.text(String(count), M + 70, y);
    y += 6;
  }
  addFooter(1);

  // ── PAGE 2: Full Job List ─────────────────────────────────────────────
  doc.addPage();
  let y2 = M + 5;
  y2 = header('Full Job List', y2);

  doc.setFontSize(8);
  const colW = [90, 22, 55, 20];
  const headers2 = ['Job', 'Status', 'Issues', 'Score'];
  headers2.forEach((h, i) => {
    doc.setFont('helvetica', 'bold');
    doc.text(h, M + colW.slice(0,i).reduce((a,b)=>a+b,0), y2);
  });
  doc.setFont('helvetica', 'normal');
  y2 += 5;

  for (const job of scanResult.jobs) {
    if (y2 > H - 20) { doc.addPage(); y2 = M + 5; addFooter(doc.getNumberOfPages()); }
    const issueStr = job.issues.map(i => i.flag).join(', ') || '—';
    const scoreStr = job.score === null ? 'n/a' : `${job.score}%`;
    doc.text(job.name.slice(0, 42), M, y2);
    doc.text(job.status, M + colW[0], y2);
    doc.text(issueStr.slice(0, 28), M + colW[0] + colW[1], y2);
    doc.text(scoreStr, M + colW[0] + colW[1] + colW[2], y2);
    y2 += 5;
  }
  ...
  doc.addPage();
  let y3 = M + 5;
  y3 = header('Issues Detail', y3);

  const flagGroups3 = {};
  for (const job of scanResult.jobs)
    for (const issue of job.issues) {
      if (!flagGroups3[issue.flag]) flagGroups3[issue.flag] = [];
      flagGroups3[issue.flag].push({ job: job.name, ...issue });
    }

  for (const [flag, items] of Object.entries(flagGroups3)) {
    if (y3 > H - 30) { doc.addPage(); y3 = M + 5; addFooter(doc.getNumberOfPages()); }
    doc.setFont('helvetica', 'bold'); doc.setFontSize(10);
    doc.text(`${flag} (${items.length})`, M, y3); y3 += 6;
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8.5);
    for (const item of items) {
      if (y3 > H - 20) { doc.addPage(); y3 = M + 5; addFooter(doc.getNumberOfPages()); }
  ...
  // tJava group
  const manualJobs3 = scanResult.jobs.filter(j => j.status === 'MANUAL');
  if (manualJobs3.length > 0) {
    if (y3 > H - 30) { doc.addPage(); y3 = M + 5; }
    doc.setFont('helvetica', 'bold'); doc.setFontSize(10);
    doc.text(`tJava — manual implementation required (${manualJobs3.length})`, M, y3); y3 += 6;
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8.5);
    for (const job of manualJobs3) {
      doc.text(`  • ${job.name}`, M, y3); y3 += 5;
    }
  }
  ...
  // Save
  const filename = `talend-migration-${projectName}-${date}.pdf`;
  doc.save(filename);
}
